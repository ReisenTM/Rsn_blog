...
// UpdateArticleView 更新文章 TODO:原文的填充
func (ArticleApi) UpdateArticleView(c *gin.Context) {
	...
	if global.Config.Site.SiteInfo.Mode == 2 {
		if user.Role != enum.RoleAdminType {
			resp.FailWithMsg("博客模式下，普通用户不能更新文章", c)
			return
		}
	}
	//文章是否存在
	var am model.ArticleModel
	err = global.DB.First(&am, "user_id = ? and id = ?", user.ID, cr.ArticleID).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		resp.FailWithMsg("文章不存在", c)
		return
	}
	//更新的文章只能是自己的
	if am.UserID != user.ID {
		resp.FailWithMsg("只能修改自己的文章", c)
		return
	}
	//判断分类id是否是自己创建
	...
	err = global.DB.Model(&am).Updates(m).Error
	if err != nil {
		resp.FailWithError(err, c)
		logrus.Errorf("更新文章-保存到数据库失败")
		return
	}
	resp.OKWithMsg("更新文章成功", c)
}
